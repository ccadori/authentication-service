...
class User {
  static async create (username, email, password, customFields, getConnection = baseGetConnection) {
  ...
  static async findByEmail (email, getConnection = baseGetConnection) {
    if (!validator.isEmail(email))
      return { error: { code: 400, message: "Invalid email." } };
